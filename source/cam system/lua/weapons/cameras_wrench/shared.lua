...
SWEP.Contact 				= ""
SWEP.Purpose				= ""
SWEP.Instructions			= "Press [IN_USE] or [IN_ATTACK] \nand look on Cam System`s entity \n[IN_ATTACK2] on ent will open admin menu"
...
SWEP.ViewModelBoneMods = {
	...
	["ValveBiped.Bip01_R_Finger4"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(-5, -60, -16) },
	["ValveBiped.Bip01_R_Hand"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(-3, 3, -55) },
	...
	["ValveBiped.Bip01_R_Finger3"] = { scale = Vector(1, 1, 1), pos = Vector(1, 0, 0), angle = Angle(3, -82, -30) },
...
SWEP.VElements = {
	["wrench"] = { type = "Model", model = "models/props_c17/tools_wrench01a.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(2.5, 1.2, 1.8), angle = Angle(200, -20, 95), size = Vector(0.8, 0.8, 0.8), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["wrench"] = { type = "Model", model = "models/props_c17/tools_wrench01a.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(2, 2, 0), angle = Angle( 120, 180, 90), size = Vector(0.8, 0.8, 0.8), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
util.AddNetworkString( 'cl_wrench_cam' )
util.AddNetworkString( 'sv_cam_unbreakable' )
util.AddNetworkString( 'sv_change_cam_type' )

net.Receive( "sv_cam_unbreakable", function() 
...
net.Receive( "sv_change_cam_type", function() 
...
net.Receive( "cl_wrench_cam", function() 
	...
	if ent:GetClass() == "ent_cam" then
		Models = Cameras.Models
	else
		Models = Cameras.Monitors
	end

	for j, k in pairs(Models) do
		local DImage = vgui.Create( "DModelPanel", frame )
		DImage:SetModel( j )
		DImage:SetSize( 120, 120 )
		DImage:SetCamPos( Vector(20,20,0) )
		DImage:SetLookAt( Vector(0,0,0) )
		DHorizontalScroller:AddPanel( DImage )
		DImage.DoClick = function()
			net.Start('sv_change_cam_type')
				net.WriteString(j)
				net.WriteEntity(ent)
			net.SendToServer()
		end
...
function SWEP:PrimaryAttack()
	...
	if ent:GetClass() == "ent_cam" or ent:GetClass() == "ent_cam_control" then
